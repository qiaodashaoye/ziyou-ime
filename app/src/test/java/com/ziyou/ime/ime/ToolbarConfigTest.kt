...
class ToolbarConfigTest {
    ...
    @Test
    fun `默认配置为四核心按钮且顺序符合预期`() {
        // 默认配置收敛为四核心：技能/粘贴板/AI 问答/人设润色（功能栏从右往左
        // 排列，视觉左起为 润色→AI→粘贴板→技能）；AI 问答与人设润色为两个
        // 独立入口相邻排布；其余按钮由用户在设置页或键盘内工具面板编辑模式
        // 自行添加；已自定义的存量用户不受默认值变更影响
        assertEquals(
            listOf("skill", "clipboard", "ai", "ai_polish"),
