 ...
 * 目标工具若为其他面板可无缝接管同一空间）。
 *
 * 兼任面板编辑模式（键盘内自定义功能栏）的配置读写出口：
 * 读取经 [ToolbarConfigLogic.sanitize] 清洗，写入直落
 * [ToolbarConfigRepository]，功能栏视图的 SharedPreferences 监听
 * 即时刷新（与设置页同一存储 key，两入口天然互通）。
 *
...
class ToolPanelCoordinator(
    ...
    private val panelHost = object : ToolPanelView.Host {
        ...
        override fun enabledToolbarIds(): List<String> =
            ToolbarConfigLogic.sanitize(
                ToolbarConfigRepository.getItemIds(service),
                ToolbarItem.ALL_IDS,
                ToolbarConfigRepository.DEFAULT_IDS
            )

        override fun onToolbarIdsChanged(ids: List<String>) {
            // 即改即存：功能栏视图经配置监听实时刷新，关面板即见新布局
            ToolbarConfigRepository.setItemIds(service, ids)
        }
    }
}
