...
/**
 * 皮肤开发文档页面：展示随 APK 打包的《自定义皮肤开发指南》。
 *
 * 文档来源：docs/自定义皮肤开发指南.md（构建时经 syncSkinDevGuide 任务拷入
 * assets/docs/skin_dev_guide.md，单一来源不双维护）。
 * 内容涵盖：皮肤包格式（.zyskin）、skin.json 配置规范、开发步骤与制作流程。
 * 渲染：core-logic 的 [SimpleMarkdown] 转 HTML 后由 WebView 展示——
 * 本页 WebView 为纯本地静态文档，JS/文件/网络访问全部关闭。
 */
class SkinDevGuideActivity : AppCompatActivity() {

    companion object {
        private const val TAG = "SkinDevGuideActivity"
        private const val GUIDE_ASSET = "docs/skin_dev_guide.md"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentViewWithTitleBar("皮肤开发文档", webView)
    ...
    private fun buildGuideHtml(): String {
        val markdown = try {
            ...
            Log.e(TAG, "皮肤开发文档读取失败: ${e.message}")
            "# 文档加载失败\n\n请重新安装应用，或访问仓库查看 docs/自定义皮肤开发指南.md"
        }
