...
object T9PinYinUtils {
    ...
    /** 反向索引：拼音 → T9键序列（组代表字母），用于 O(1) 查找 */
    private val reverseMap = HashMap<String, String>(400)

    /**
     * 数字键（2-9）→ 组代表字母（A/D/G/J/M/P/T/W）。
     *
     * [pinyinMap] / [reverseMap] 以「组代表字母」为键，而键盘与 Rime 输入串
     * 均以「数字键」为表示（如 486）。为保证全链路一致，本工具类对外
     * API 统一以「数字键」为规范表示，仅在内部查表时转换为组代表字母。
     */
    private val digitToGroup = mapOf(
        '2' to 'A', '3' to 'D', '4' to 'G', '5' to 'J',
        '6' to 'M', '7' to 'P', '8' to 'T', '9' to 'W'
    )

    /** 组代表字母 → 数字键（[digitToGroup] 的逆映射） */
    private val groupToDigit = digitToGroup.entries.associate { (d, g) -> g to d }
     ...
     * 获取 T9 数字键序列对应的候选拼音。
     *
     * @param t9Sequence T9 数字键序列（如 "486"，对应键 4-8-6）
     * @return 所有可能拼音（按匹配长度由长到短、去重）；无匹配返回空数组
     */
    fun t9KeyToPinyin(t9Sequence: String?): Array<String> {
        if (t9Sequence.isNullOrEmpty()) return emptyArray()
        // 数字键序列先转换为 pinyinMap 使用的组代表字母序列
        val groupSeq = digitsToGroups(t9Sequence)
        if (groupSeq.isEmpty()) return emptyArray()
        val limited = if (groupSeq.length > 6) groupSeq.substring(0, 6) else groupSeq
        // 由长到短匹配前缀，优先展示更完整的音节；LinkedHashSet 保证去重且保序
        val pinyin = LinkedHashSet<String>()
        for (length in limited.length downTo 1) {
            val prefix = limited.substring(0, length)
            pinyinMap[prefix]?.split(",")?.forEach { pinyin.add(it) }
        }
        return pinyin.toTypedArray()
    }

    /**
     * 获取拼音对应的 T9 数字键序列（O(1) 查找）。
     *
     * @param sequence 拼音字符串（如 "guo"）
     * @return 对应的 T9 数字键序列（如 "486"）；未找到返回空字符串
     */
    fun pinyin2Key(sequence: String?): String {
        if (sequence.isNullOrEmpty()) return ""
        val groupSeq = reverseMap[sequence] ?: return ""
        return groupsToDigits(groupSeq)
    }

    /** 数字键序列 → 组代表字母序列；含非 2-9 字符时返回空串 */
    private fun digitsToGroups(digits: String): String {
        val sb = StringBuilder(digits.length)
        for (c in digits) sb.append(digitToGroup[c] ?: return "")
        return sb.toString()
    }

    /** 组代表字母序列 → 数字键序列；含非法字母时返回空串 */
    private fun groupsToDigits(groups: String): String {
        val sb = StringBuilder(groups.length)
        for (c in groups) sb.append(groupToDigit[c] ?: return "")
        return sb.toString()
    }
    ...
    /**
     * 将T9编码格式化为可读拼音显示
     * @param composition T9编码组合
     * @param comment 拼音注释（用于对照）
     * @return 格式化后的拼音字符串
     */
