...
## 更新摘要
**所做更改**   
- 更新了 Bridge API 版本说明，反映 v2 最终版和实验性 v3/v4 stats/IME API 的回滚
- 修正了核心组件分析中的 API 版本信息
- 更新了依赖分析以反映稳定的 API 设计
- 保持了整体架构的一致性和准确性
...
**重要更新**：当前架构基于稳定的 Bridge API v2 最终版，已回滚实验性的 v3/v4 stats/IME API，确保系统的稳定性和可维护性。
...
- 核心逻辑
  - RimeApi：对外暴露的 Rime 能力接口（基于稳定 v2 API）
...
**重要更新**：核心 API 已稳定在 v2 版本，移除了不稳定的 v3/v4 stats/IME API 实验性功能，确保生产环境的稳定性。
...
AssociationManager <.. SimpleRimeInputMethodService : "使用"
...
### 原生层（JNI 桥接）
- rime_jni.cc：JNI 入口，导出方法供 Kotlin 调用（基于稳定 v2 API）
- session.h：会话对象封装，管理 librime 上下文
- config.cc：配置读取与写入
- helper-types.h/jni-utils.h/objconv.h：类型转换与工具函数

**重要更新**：JNI 接口已稳定在 v2 版本，移除了实验性的 stats/IME API，确保跨语言调用的稳定性。

```mermaid
sequenceDiagram
participant Kotlin as "Kotlin 层"
participant JNI as "JNI 桥接 (v2 API)"
...
- 外部依赖
  - librime：C++ 引擎，通过 JNI 调用（基于稳定 v2 API）
  - Android IME 框架：输入法服务与视图生命周期管理
- 潜在循环依赖
  - 通过接口与消息传递避免 UI 与核心逻辑的直接耦合

**重要更新**：依赖关系已稳定，移除了对实验性 API 的依赖，确保系统的长期可维护性。
...
- API 稳定性：基于 v2 稳定版本，避免实验性 API 的性能波动
...
## 结论
ziyou-ime 采用清晰的分层架构与模块化设计，UI 层专注交互，核心逻辑封装 Rime 引擎，数据层提供轻量结构，原生层保障高性能。通过接口与消息传递实现松耦合，便于扩展与维护。当前架构基于稳定的 Bridge API v2 最终版，已回滚实验性的 v3/v4 stats/IME API，确保系统的稳定性和可维护性。建议在后续迭代中继续强化配置热更新、渲染优化与错误恢复机制。
...
- 扩展点与自定义机制
  ...
  - 原生扩展：在 rime_jni.cc 中新增 JNI 方法，暴露 librime 能力（需遵循 v2 API 规范）

**重要更新**：所有扩展点都应遵循稳定的 v2 API 规范，避免引入实验性 API 导致的不稳定性。