...
## 更新摘要
**所做更改**   
- 新增 RimeDeployStep 抽象层，解耦守护进程层与业务域依赖
- 更新 AppContainer 组合根处理装配逻辑
- 增强部署步骤的可扩展性和可测试性
- 优化配置管理的架构设计
...
**更新** 新增了 RimeDeployStep 抽象层，用于解耦守护进程层与业务域依赖，提升了系统的可维护性和可扩展性。
...
- app/src/main/java/com/ziyou/ime/daemon：守护进程相关组件，包含新的 RimeDeployStep 抽象
...
D["AppContainer<br/>组合根装配"]
end
subgraph "守护进程层"
E["RimeDeployStep<br/>部署步骤抽象"]
F["RimeSession<br/>会话管理"]
end
subgraph "资源层"
G["assets/rime/*.yaml<br/>默认配置"]
end
subgraph "JNI/底层"
H["rime_jni.cc<br/>JNI 入口"]
I["config.cc<br/>配置相关"]
J["librime<br/>配置编译/合并/校验"]
end
A --> C
A --> E
A --> G
...
- **新增** RimeDeployStep：定义部署步骤的抽象接口，解耦守护进程层与业务域依赖，支持灵活的部署流程编排。
...
## 架构总览
整体流程：应用启动时由 AssetDeployer 部署默认 YAML；RimeConfigManager 通过 RimeDeployStep 抽象层协调部署步骤，读取并解析 YAML，通过 JNI 调用底层 librime 完成配置编译与校验；ThemeManager 基于当前 schema 与主题名加载主题资源并在 UI 层生效。
...
App->>Container : "应用启动"
Container->>Deployer : "初始化部署器"
Container->>Step : "注册部署步骤"
Container->>ConfigMgr : "初始化配置管理器"
Deployer-->>App : "部署完成"
ConfigMgr->>Step : "执行部署步骤"
Step-->>ConfigMgr : "步骤执行结果"
...
- 职责
  ...
  - 通过 RimeDeployStep 抽象层协调部署流程，避免直接依赖守护进程层。
...
CheckDir --> |缺失| Deploy["通过RimeDeployStep部署默认配置"]
...
- 职责
  - 定义部署步骤的统一接口，支持多种部署策略的实现。
  - 解耦守护进程层与业务域依赖，提高代码的可测试性和可维护性。
  - 提供步骤链式调用和错误处理机制。
- 关键特性
  - 抽象接口设计：定义 execute() 方法作为标准部署流程。
  - 错误传播：统一的异常处理机制，支持步骤间错误传递。
  - 状态管理：维护部署过程中的中间状态和结果。
- 扩展性
  - 支持自定义部署步骤实现，如数据库迁移、配置文件转换等。
  - 支持步骤条件执行和并行执行。

```mermaid
classDiagram
class RimeDeployStep {
+execute(context) DeploymentResult
+getName() string
+getVersion() int
}
class AssetDeploymentStep {
+execute(context) DeploymentResult
}
...
RimeDeployStep <|-- AssetDeploymentStep
RimeDeployStep <|-- ConfigMigrationStep
RimeDeployStep <|-- ValidationStep
```
...
## 依赖关系分析
- 组件耦合
  - RimeConfigManager 依赖 AssetDeployer 完成初始部署，通过 RimeDeployStep 抽象层协调部署流程，避免直接依赖守护进程层。
  - ThemeManager 依赖文件系统与 UI 资源，与 RimeConfigManager 解耦但共享 schema 信息。
  - AppContainer 作为组合根，负责所有组件的装配和生命周期管理。
- 外部依赖
  - librime 提供配置编译、合并与校验能力。
  - Android 存储 API 用于读写用户数据目录。
- 潜在循环依赖
  - 通过接口隔离避免循环，如配置加载与主题切换独立。
  - RimeDeployStep 抽象层消除了守护进程层与业务层的直接依赖。

```mermaid
graph LR
RCM["RimeConfigManager"] --> AD["AssetDeployer"]
RCM --> RDS["RimeDeployStep"]
RDS --> RS["RimeSession"]
...
- **新增** 部署步骤优化
  - RimeDeployStep 支持步骤级缓存，避免重复执行相同步骤。
  - 支持步骤并行执行，提升部署效率。
...
- 常见问题
  ...
  - 部署步骤失败：检查 RimeDeployStep 实现和错误处理逻辑。
- 调试步骤
  - 启用详细日志，观察配置编译与校验过程。
  - 对比 assets 默认配置与用户目录差异，定位异常。
  - 逐步禁用插件或过滤器，缩小问题范围。
  - 检查 AppContainer 的组件装配是否正确。
...
## 结论
本配置管理系统通过清晰的组件划分与 JNI 桥接，实现了高效的配置加载、解析与热更新。新增的 RimeDeployStep 抽象层成功解耦了守护进程层与业务域依赖，提升了系统的可维护性和可扩展性。ThemeManager 提供了灵活的主题管理能力。遵循 YAML 规范与优先级规则，结合完善的错误处理与性能优化，确保了系统的稳定性与可扩展性。
...
- 配置迁移
  - 版本号控制，升级时执行迁移脚本。
  - 保持向后兼容，废弃字段标记为可选。
- **新增** RimeDeployStep 扩展指南
  - 实现 RimeDeployStep 接口定义部署逻辑。
  - 在 AppContainer 中注册自定义部署步骤。
  - 处理步骤间的依赖关系和执行顺序。